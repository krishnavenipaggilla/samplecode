...
public class AuthorizedRepresentativePage {
	ConfigReader config = new ConfigReader();

	WebDriver driver;
	Logger Log;

	public AuthorizedRepresentativePage(WebDriver ldriver) {
		this.driver = ldriver;
	}

	public AuthorizedRepresentativePage(Logger lLog) {
		this.Log = lLog;
	}

	public void verifyAuthorizedRepresentativePage() throws Exception {
		...
		/*
		 * lnk.clickMemberActionCenter(); try {
		 * addAuthorizedRepresentativeFromOverview();
		 * Log.info("Added the auth rep from Overview");
		 * 
		 * } catch (Exception e) { e.printStackTrace(); passed = false; }
		 * 
		 * /* try { verifyAddedAuthorizedRepresentativeFromOverview();
		 * 
		 * Log.info("Verified the auth rep from Overview"); } catch (Exception
		 * e) { e.printStackTrace(); passed = false; }
		 * 
		 * try { editAndVerifyAuthorizedRepFromOverview();
		 * Log.info("Edit and Verify the auth rep from Overview"); } catch
		 * (Exception e) { e.printStackTrace(); passed = false; }
		 */
		lnk.clickMemberActionCenter();
		
		try {
			Log.info("Add auth rep from initial outreach page");
			addAuthorizedRepresentativeFromInitialOutreachPage();
		} catch (Exception e) {
			e.printStackTrace();
			passed = false;
		}
		...
		try {
			Log.info("Verify added auth rep from initial outreach page");
			verifyAddedAuthorizedRepresentativeFromInitialOutreachPage();
		...
		try {
			addAndVerifyDateFromGridForSingleAuthReps();
			Log.info("Verified the single auth rep verify date columns");
		} catch (Exception e) {
			e.printStackTrace();
			passed = false;
		}
		...
		try {
			addAndVerifyDateFromGridForMultipleAuthReps();
			Log.info("Verified the multiple auth rep verify date columns");
		} catch (Exception e) {
			e.printStackTrace();
			passed = false;
		}
		...
		try {
				Log.info("Verify pagination for auth rep grid from Initial Outreach");
				numberOfRowsInGridForpagination("Authorized Rep(s)", 10);
		} catch (Exception e) {
				e.printStackTrace();
				passed = false;
		}
...
/*		try {
				Log.info("Verify column level sorting on auth rep grid from overview");
				columnLevelSortingForAuthRepFromOverview();
		...
		try {
			lnk.clickOverview();
			Log.info("Verify pagination for auth rep grid from overview");
			numberOfColumnsInGridForpagination("Member Representatives", 5);
...
*/
	...
	public void addAuthorizedRepresentativeFromOverview() throws Exception {
		Log = Logger.getLogger("AuthorizedRepresentativePage.class");
		PropertyConfigurator.configure("log4j.properties");
	...
	public void verifyAddedAuthorizedRepresentativeFromOverview() throws Exception {
		Log = Logger.getLogger("AuthorizedRepresentativePage.class");
		PropertyConfigurator.configure("log4j.properties");
		WebDriverWait wait = new WebDriverWait(driver, GlobalValues.Explicit_Wait_time);
		Global_Method gm = new Global_Method(driver);
	...
	public void addAuthorizedRepresentativeFromInitialOutreachPage() throws Exception {
		Log = Logger.getLogger("AuthorizedRepresentativePage.class");
		PropertyConfigurator.configure("log4j.properties");
		AnnualAssessment a = new AnnualAssessment(driver);
		WebDriverWait wait = new WebDriverWait(driver, GlobalValues.Explicit_Wait_time);
	...
	public void verifyAddedAuthorizedRepresentativeFromInitialOutreachPage() throws Exception {
		Log = Logger.getLogger("AuthorizedRepresentativePage.class");
		PropertyConfigurator.configure("log4j.properties");
		WebDriverWait wait = new WebDriverWait(driver, GlobalValues.Explicit_Wait_time);
		Global_Method gm = new Global_Method(driver);
	...
	public void addAndVerifyDateFromGridForMultipleAuthReps() throws Exception {
		Log = Logger.getLogger("AuthorizedRepresentativePage.class");
		PropertyConfigurator.configure("log4j.properties");
		AnnualAssessment a = new AnnualAssessment(driver);
		WebDriverWait wait = new WebDriverWait(driver, GlobalValues.Explicit_Wait_time);
	...
	public void addAndVerifyDateFromGridForSingleAuthReps() throws Exception {
		Log = Logger.getLogger("AuthorizedRepresentativePage.class");
		PropertyConfigurator.configure("log4j.properties");
		AnnualAssessment a = new AnnualAssessment(driver);
		WebDriverWait wait = new WebDriverWait(driver, GlobalValues.Explicit_Wait_time);
	...
	public void columnLevelSortingForAuthRepFromOverview() throws Exception {
		...
		try {
			...
			Log.info(gridTitle);
			Assert.assertEquals(gridTitle, "Member Representatives", "Grid Header is not matching");
			...
			Log.info(e + " - sorting Member Representative grid failed");
		}
	...
	public void numberOfRowsInGridForpagination(String gridName, int requiredRowCount) throws Exception {
		...
		try {
			...
			String gridTitle = gridTitleElement.getText();
			Log.info(gridTitle);
			//Assert.assertEquals(gridTitle, "Authorized Rep(s)", "Grid Header/Title is not matching");
	...
	public void editAndVerifyAuthorizedRepFromOverview() throws Exception {
		Log = Logger.getLogger("AuthorizedRepresentativePage.class");
		PropertyConfigurator.configure("log4j.properties");
		AnnualAssessment a = new AnnualAssessment(driver);
		WebDriverWait wait = new WebDriverWait(driver, GlobalValues.Explicit_Wait_time);
