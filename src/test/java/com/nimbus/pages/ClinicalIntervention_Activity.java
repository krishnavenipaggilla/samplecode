...
public class ClinicalIntervention_Activity {
	WebDriver driver;
	Logger Log;
	ConfigReader config = new ConfigReader();
	WebElement ClinicalInterventionActivityStatus = null;
	String ClinicalInterventionStatus = null;
	List<String> actualsubsections = null;
	LeftNavigationLink leftNav=null;
	
	public ClinicalIntervention_Activity(WebDriver ldriver) {
		this.driver = ldriver;
	}

	public ClinicalIntervention_Activity(Logger lLog) {
		this.Log = lLog;
	}
	public void clinicalIntervention() throws Exception {
		Log = Logger.getLogger("ClinicalIntervention_Activity.class");
...
//		mac.addActivity("Clinical Intervention");
		
		ClinicalInterventionActivityStatus =wait.until(ExpectedConditions.elementToBeClickable(By.xpath(config.getClinicalInterventionActivitystatus())));
		ClinicalInterventionStatus = ClinicalInterventionActivityStatus.getText().toString().trim();
		Log.info("ClinicalInterventionstatus---" + ClinicalInterventionStatus);
		if (ClinicalInterventionStatus.equals(GlobalValues.Open_status.toString())
				|| ClinicalInterventionStatus.equals(GlobalValues.InProgress_status.toString()))

		{
			...
			for (int j = 0; j < options.size(); j++) {
				...
				actualsubsections=GlobalValues.ClinicalIntervention_DefaultSubsections;
				}
			Log.info("verifying actual and expected default subsections in clinical intervention");
			GM.validateSubsections(actualsubsections, expctedsubsections);
			Log.info("verified actual and expected default subsections in clinical intervention");
	...
	public void verifyAllSubsections() {
		...
		actualsubsections=GlobalValues.ClinicalIntervention_AllSubsections;
		Log.info("verifying actual and expected subsections in clinical intervention after adding all subsections");
		GM.validateSubsections(actualsubsections, expctedsubsections);
		Log.info("verified actual and expected subsections in clinical intervention after adding all subsections");
//		fillSubsections();
	}
